...
public class GasBottleCalc {
    ...
    public GasBottleCalc(GasBottle gasBottle) {
        ...
        Rg = gasBottle.getRg();
        CA = gasBottle.getCA();
        p.add(gasBottle.getP0());
        temp.add(gasBottle.getTemp0());
        rho.add(gasBottle.getP0()/Rg/gasBottle.getTemp0());
    }
    ...
    public void execute(double pe){
        int index = p.size()-1;
        if (q.isEmpty()){
            q.add(getQ(gasBottle.getP0(), rho.get(0), pe));
        }
        //气瓶组件
