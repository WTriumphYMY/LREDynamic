...
import cn.edu.nwpu.utils.CalculateUtils;
...
public class ReducingValveCalc {
    private ReducingValve reducingValve;
    ...
    public ReducingValveCalc(ReducingValve reducingValve) {
        this.reducingValve = reducingValve;
        ...
        q_high.add(0.0);
        q_low.add(0.0);
        h = GlobleParas.TIME_STEP;
    }

    public void execute(double qin, double tin, double pe){
        int index = p_high.size()-1;
//        //减压阀组件
        dx[0] = getDx(u.get(index), x.get(index));
        du[0] = getDu(p_high.get(index), p_low.get(index), x.get(index), dx[0]);
        dp1[0] = getDp1(temp_high.get(index), q_high.get(index), p_high.get(index), dx[0], qin, tin);
        dp2[0] = getDp2(q_high.get(index), temp_high.get(index), q_low.get(index), temp_low.get(index), p_low.get(index), dx[0]);
        dtemp1[0] = getDt1(temp_high.get(index), q_high.get(index), p_high.get(index), dp1[0], qin, dx[0]);
        ...
        for (int i = 1; i < 4; i++){
           ...
           //减压阀
            ...
            du[i] = getDu(p_high.get(index)+ratio*h*dp1[i-1], p_low.get(index)+ratio*h*dp2[i-1],
                    x.get(index)+ratio*h*dx[i-1],dx[i-1]);
            dp1[i] = getDp1(temp_high.get(index)+ratio*h*dtemp1[i-1],
                    ...
                    p_high.get(index)+ratio*h*dp1[i-1], dx[i-1],qin, tin);
            dp2[i] = getDp2(getQ(p_high.get(index)+ratio*h*dp1[i-1], p_low.get(index)+ratio*h*dp2[i-1],
                    ...
                    getQ(p_low.get(index)+ratio*h*dp1[i-1], 101325.0, temp_low.get(index)+ratio*h*dtemp2[i-1]),
            ...
            dtemp1[i] = getDt1(temp_high.get(index)+ratio*h*dtemp1[i-1],
                    getQ(p_high.get(index)+ratio*h*dp1[i-1], p_low.get(index)+ratio*h*dp2[i-1],
                            temp_high.get(index)+ratio*h*dtemp1[i-1]),
                    p_high.get(index)+ratio*h*dp1[i-1], dp1[i-1], qin, dx[0]);
            dtemp2[i] = getDt2(getQ(p_high.get(index)+ratio*h*dp1[i-1], p_low.get(index)+ratio*h*dp2[i-1],
                    temp_high.get(index)+ratio*h*dtemp1[i-1]),
                    getQ(p_low.get(index)+ratio*h*dp2[i-1], 101325.0, temp_low.get(index)+ratio*h*dtemp2[i-1]),
    ...
    public double getDu(double p1, double p2, double x, double dx){
        //TODO 阀芯运动方程还需研究
        double du;
        du = p1*Avc/M + p2*(Amb-Avc)/M - 101325.0*Amb/M - C*dx/M - stiffness*(x+0.009)/M - 0*F/M;
        if (x >= xstop && du > 0){
            du = 0;
        }
        if (x <= 0 && du < 0){
     ...
     * @return 高压腔压强导数
     */
    public double getDp1(double t1, double q1, double p1, double dx, double qin, double tin){
        double dp1;
        dp1 = k*Rg*(qin*tin-q1*t1)/V1 - k*p1*Avc*dx/V1;
        return dp1;
    }
     ...
     * @return 高压腔温度导数
     */
    public double getDt1(double t1, double q1, double p1, double dp1, double qin, double dx){
        double dt1;
        dt1 = t1*dp1/p1 + t1*Avc*dx/V1 - Rg*t1*t1*(qin-q1)/p1/V1;
        return dt1;
    }
    ...
    public double getDp2(double q1, double t1, double q2, double t2, double p2, double dx){
        double dp2;
        dp2 = k*Rg*(q1*t1-q2*t2)/V2 - k*p2*(Amb-Avc)*dx/V2;
        return dp2;
    }
    ...
    public double getDt2(double q1, double q2, double t2, double p2, double dp2, double dx){
        double dt2;
        dt2 = t2*dp2/p2 + t2*(Amb-Avc)*dx/V2 - Rg*t2*t2*(q1-q2)/p2/V2;
        return dt2;
    }
